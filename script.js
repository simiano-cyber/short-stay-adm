...
// =====================
// FILA DE RETRIES - GARANTIR DADOS NÃƒO SÃƒO PERDIDOS
// =====================
...
class FilaSheets {
  ...
  carregarFila() {
    ...
    if (salva) {
      try {
        ...
        console.log(`[FilaSheets] Carregadas ${this.filaLocal.length} requisiçÃµes pendentes`);
      } catch (e) {
  ...
  adicionar(acao, dados) {
    ...
    this.salvarFila();
    console.log(`[FilaSheets] Adicionado Ã  fila (pendentes: ${this.filaLocal.length})`, item.id);
  }
  ...
  async processar() {
    ...
    if (processados > 0) {
      console.log(`[FilaSheets] Processadas ${processados} requisiçÃµes da fila`);
    }
  }
...
const filaSheetsLocal = new FilaSheets();
  ...
  "Stúdio moderno Campo Belo próx. Aeroporto e MetrÃ´": {
...
function renderizarFinanceiro() {
  ...
  financeiroList.innerHTML = lancamentos
    .map((item) => {
      const valorClasse = Number(item.valor) < 0 ? "valor-saida" : "valor-entrada";
      ...
      return `
        <article class="financeiro-item">
          <div class="financeiro-item-header">
            ...
            <span data-label="Total" class="financeiro-valor ${valorClasse}">${formatarMoeda(item.valor || 0)} <small class="financeiro-origem-badge">${item.origem === "manual" ? "manual" : "automático"}</small></span>
          </div>
        </article>
      `;
    })
...
async function listarFluxoCaixaSheets() {
  try {
    ...
    console.error("Erro ao listar fluxoCaixa no Sheets:", data?.erro || "Resposta inv�lida");
    return [];
  } catch (erro) {
...
function normalizarApartamentoSheets(item) {
  ...
  const toBool = (valor, fallback = true) => {
    ...
    if (["false", "0", "nao", "n�o", "no"].includes(texto)) return false;
    return fallback;
  };
...
async function salvarFluxoCaixaSheets(item) {
  ...
  try {
    ...
    if (data?.sucesso === false) {
      console.error("Erro ao salvar fluxoCaixa no Sheets:", data?.erro || "Resposta inv�lida");
      return false;
    }
...
async function salvarLimpezaSheets(limpeza) {
  try {
    ...
    console.error("Erro ao salvar no Sheets (adicionando Ã  fila):", erro);
...
async function atualizarLimpezaSheets(limpeza) {
  try {
    ...
    console.error("Erro ao atualizar no Sheets (adicionando Ã  fila):", erro);
